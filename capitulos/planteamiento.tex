...
El prototipo desarrollado por la Universidad de Salerno cuenta con seis motores paso a paso, dos enfrentados para el movimiento horizontal, y cuatro enfrentados dos a dos para el vertical. Estos están integrados en una placa de expansión, CNC-Shield, que permiten el acoplamiento de drivers, en este caso A4988. Esta metodología es muy utilizada para fresadoras CNC, impresoras 3D, grabadoras láser, u otros usos que requieran de desplazamiento en ejes cartesianos. También se entregaron seis finales de carrera que no estaban integrados en la maqueta y que deberán de ajustarse a las necesidades de este proyecto. El control se realizó en una ESP32-Wroover, con un programa que permitía elegir la posición absoluta a la que se movía el plano de la ventana móvil a la posición introducida en una interfaz HTML cliente-servidor en el microcontrolador. Los motores vienen alimentados con una fuente de 12 V y 6 A conectada a la instalación eléctrica. Estas características imponen una serie de condicionantes técnicos que deben ser tenidos en cuenta en el diseño del sistema de control y en la selección de la plataforma hardware. 
Cabe destacar que el control, microcontrolador y entorno de programación desarrollado y empleado para la integración del sistema por la Universidad de Salerno no tiene ningún peso sobre la elección de de la plataforma seleccionada, pues se basa en un análisis riguroso de distintos dispositivos, centrándose en el cumplimiento de los requisitos funcionales y no funcionales atendiendo a una serie de criterios de selección tecnológica.

\section{Planteamiento de requisitos y selección tecnológica}
...
cubra las necesidades de caracterización, permitiendo al usuario tener un completo control del movimiento del panel móvil de la ventana. Se diferencia a continuación entre requisitos funcionales, aquellos que describen qué debe hacer el sistema, y no funcionales, que definen cómo debe hacerse.
...
\begin{itemize}
	...
	\item El sistema debe disponer de una interfaz de usuario que permita establecer la configuración de la ventana, restablecer sus valores predeterminados, cambiar entre los distintos modos y visualizar el estado de la parte móvil, es decir, en qué estado se encuentra, en qué posición está y su configuración, así como la fecha y hora.
	\begin{enumerate}
		\item \textbf{Modo automático}: es el responsable de realizar el seguimiento de la radiación directa concentrada por las lentes durante el día solar. Para ello se emplean tres algoritmos distintos que permiten calcular la posición correspondiente a la que debe dirigirse el marco móvil.
			\begin{enumerate}
				...
				partiendo de la matriz obtenida mediante \ref{sec:matriz}, donde la precisión es de un grado, se realiza una interpolación bilineal entre dicha matriz y los ángulos de incidencia obtenidos.
			\end{enumerate} 
...
La selección hardware del sistema mecatrónico ha sido realizada por la Universidad de Salerno, tal como se indica en la sección~\ref{sec:herramientas}. Por lo tanto, ha de tenerse en cuenta la compatibilidad de los posibles dispositivos hardware y plataformas software con los elementos existentes como criterio de obligado cumplimiento.

La selección tecnológica se ha confeccionado atendiendo a una serie de pautas derivadas de los requisitos funcionales y no funcionales definidos anteriormente. Asimismo, debe de ser compatible con los componentes Siguiendo el carácter proyectual de la elaboración de un prototipo orientado a la investigación, se han priorizado los criterios de flexibilidad, pues puede requerirse exportar el proyecto a un entorno distinto, hacer adecuaciones para una nueva maqueta, cambiar funcionalidades o algoritmos de operación; facilidad de integración, pues se requiere de un sistema unificado que aglutine todos las funcionalidades requeridas de una manera coherente; y rapidez de desarrollo frente a la exploración de soluciones altamente optimizadas a nivel industrial.
...
Desde el punto de vista de la portabilidad, el diseño desarrollado en FPGA presenta una fuerte dependencia del dispositivo y del fabricante. Por este motivo, la reutilización del sistema en otros entornos resulta limitada sin un esfuerzo de adaptación significativo.
...
Una vez definida la plataforma de desarrollo y el entorno de programación, se realiza una lista de todos los elementos que son necesarios para el correcto funcionamiento del programa de control y que deben de ser integrados en el sistema.
...
\begin{itemize}
	...
	\item Motores paso a paso \textbf{NEMA-17}, empleados para producir el movimiento del panel móvil.
	\item Finales de carrera, para detectar los límites físicos de la maqueta que restringen el movimiento del panel móvil.
...
\begin{figure}[t]
	...
	\subfloat[GPS NEO-6M.]{
		\includesvg[width=0.45\linewidth]{figuras/gpsneo}	
	}	
	
	\subfloat[Nema-17 42x42x38]{
		\includesvg[width=0.45\linewidth]{figuras/nema15}		
	}
	\subfloat[Nema-17 42x42x48mm]{
		\includesvg[width=0.45\linewidth]{figuras/nema17}		
	}
	
	\subfloat[CNC-Shield-V3 con drivers A4998.]{
		\includesvg[width=0.45\linewidth]{figuras/CNC_ali}			
	}
	\subfloat[Convertidor DC-DC Buck.]{
		\includesvg[width=0.45\linewidth]{figuras/buck}			
	}
	\caption{Elementos hardware. Fuente: web AliExpress.}
\end{figure}
